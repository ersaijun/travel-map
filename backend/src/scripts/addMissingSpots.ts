...
async function addMissingSpots() {
  try {
    // 连接数据库
    ...
    // 添加缺失的景区
    let addedCount = 0;
    for (const spot of missingSpotsDetails) {
      const existing = await ScenicSpot.findOne({ name: spot.name });
      if (!existing) {
        await ScenicSpot.create(spot);
        addedCount++;
        console.log(`已添加: ${spot.name}`);
      } else {
        console.log(`已存在: ${spot.name}`);
      }
    }

    console.log(`\n成功添加 ${addedCount} 个景区到数据库`);

    // 保存到文件
    const outputPath = path.join(__dirname, '../../data/missing-spots-full.json');
    await fs.writeFile(outputPath, JSON.stringify(missingSpotsDetails, null, 2), 'utf-8');
    console.log(`\n已保存到 ${outputPath}`);
