...
async function createCompleteDataFile() {
  try {
    // 读取现有的详细数据
    const existingDataPath = 'c:/work/code/vibe-coding/travel-map/data/scenic-spots-detail-full.json';
    const existingData = JSON.parse(await fs.readFile(existingDataPath, 'utf-8'));

    console.log(`现有数据文件中有 ${existingData.length} 个景区`);

    // 合并数据
    const mergedData = [...existingData, ...newSpotsDetail];
    console.log(`合并后共有 ${mergedData.length} 个景区`);

    // 写入文件
    await fs.writeFile(existingDataPath, JSON.stringify(mergedData, null, 2), 'utf-8');
    console.log(`已更新 ${existingDataPath}`);

    // 同时更新基本信息文件
    const basicData = mergedData.map(spot => ({
      name: spot.name,
    ...
    console.log(`已更新 scenic-spots.json`);
